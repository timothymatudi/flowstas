...
// GET /api/gitlab/callback — GitLab redirects here after the user authorizes.
// We verify the CSRF state, swap the code for a token, store it (encrypted), and
// send the user back where they started with ?gitlab=connected.
export async function GET(req: Request) {
  ...
  const match = cookie.match(/(?:^|;\s*)gl_oauth_state=([^;]+)/)
  ...
  const fail = (reason: string) => {
    ...
    dest.searchParams.set('gitlab', reason)
    const r = NextResponse.redirect(dest)
    r.cookies.set('gl_oauth_state', '', { path: '/', maxAge: 0, domain: cookieDomain })
    return r
  }
  ...
  const dest = new URL(returnTo || '/deploy', origin)
  dest.searchParams.set('gitlab', 'connected')
  const res = NextResponse.redirect(dest)
  res.cookies.set('gl_oauth_state', '', { path: '/', maxAge: 0, domain: cookieDomain })
  return res
}
